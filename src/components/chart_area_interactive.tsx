...
export function ChartAreaInteractive({
  ...
  React.useEffect(() => {
    ...
    const dataHash = JSON.stringify(vuData.map(vu => ({ 
      id: vu.vu_id, 
  ...
  }, [vuData]); // Removed setChartHistory from dependencies
  ...
  const miniChartsToShow = showAllMiniCharts ? miniChartsData : miniChartsData.slice(0, 4);
            ...
            <div className="mb-4 flex justify-between items-center">
              <h3 className="text-sm font-medium text-gray-700">
                ...
                <span className="text-gray-500 font-normal ml-1">
                  ({showAllMiniCharts ? miniChartsData.length : Math.min(4, miniChartsData.length)} of {miniChartsData.length})
                </span>
              </h3>
              <Button
                variant="outline"
            ...
            {miniChartsToShow.map(({ key, label, data }, index) => {
              ...
              return (
                <Card key={key} className="h-[180px] overflow-hidden">
                  <CardHeader>
                    <CardTitle className="font-medium truncate leading-tight -m-2" title={label}>
                      {label}
                    </CardTitle>
                  </CardHeader>
